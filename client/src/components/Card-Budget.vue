<template>
  ...
  export default {
    ...
    computed: {
      remainingBudget() {
        return Math.max(this.availableBudget - this.expensesToday, 0);
      },
      progressPercentage() {
        return (this.expensesToday / this.availableBudget) * 100;
      },
      progressColorClass() {
        if (this.progressPercentage < 30) return 'progress-low';
        if (this.progressPercentage < 60) return 'progress-medium';
