...
const eventsModule = {
  ...
  mutations: {
    ...
    ADD_EVENT(state, event) {
      const index = state.events.findIndex(e => e.title === event.title && e.date === event.date && e.type === event.type && e.description === event.description);
      if (index === -1) {
        state.events.push(event);
      }
      else{
        throw new Error('Evenimentul există deja!');
      }
    },
    UPDATE_EVENT(state, eventData ) {
      const index = state.events.findIndex(e => e.title === event.title && e.date === event.date && e.type === event.type && e.description === event.description);
      if (index !== -1) {
